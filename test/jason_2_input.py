...
class jason_reader():
    def __init__(self, jason_path, time_step):
    ...
    def read_folder(self):
        # print('我在哪', os.getcwd(), os.getcwd().split('\\')[-1])
        if os.getcwd().split('\\')[-1] != 'test':
            self.jason_out_path = self.jason_path
        else:
            self.jason_out_path = self.jason_path.split('/')[-1]
        js_names = []
        for js in os.listdir(self.jason_out_path):
            #print('====================>>' + js + '<<====================')
            js_names.append(js)
        # if len(js_names[-self.time_step:]) >= 20:
        #     print('gesture', len(js_names[-self.time_step:]), js_names[-1])
        return js_names[-self.time_step:]


    def run(self):
        # last 20 files
        activity_names = self.read_folder()
        gesture_per_person = []
        count = 0
        gesture_reference = np.ones((15, 2))
        for js in activity_names:
            with open(os.path.join( self.jason_out_path, js)) as json_file:
                #print('====================>>' + js + '<<====================')
                ...
                if len(np.shape(joints_centered)) == 2 and np.shape(joints_centered)[1]==0:
                    ...
                    #     print(str(count), np.shape(gesture_per_person))
                    #     #gesture_per_person = []
                    #     #count += 1
                else:
                    ...
                    if not self.nan_in_the_list(normalized_feature) and self.num_empty < 5:
        ...
        return gesture_per_person



if __name__ == '__main__':
    jason_path = 'outputs_1'
    c = jason_reader(jason_path, time_step=20)
    c.read_folder()
